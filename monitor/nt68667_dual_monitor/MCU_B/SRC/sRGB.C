...
//------------------ +jwshin 111208   실제 사용 루틴...
void SetContrastRGB(void)
{
#define USER_GAIN_MAX	255
#define USER_GAIN_MIN 	127
#define USER_GAIN_RANGE 128
unsigned char R_OFFSET_MID,G_OFFSET_MID,B_OFFSET_MID;
...
//printf("RGB0=%d,%d,%d\r\n",(unsigned short)tR,(unsigned short)tG,(unsigned short)tB);
	tR = (unsigned int)(((double)ucRGB_OFFSET+((double)150*ucContrast/100))*((double)tR/255)); //20060313 Steward modified //Set 250 in center point (Contrast=50%)
	tG = (unsigned int)(((double)ucRGB_OFFSET+((double)150*ucContrast/100))*((double)tG/255)); //20060313 Steward modified
	tB = (unsigned int)(((double)ucRGB_OFFSET+((double)150*ucContrast/100))*((double)tB/255));  //20060313 Steward modified
//printf("RGB1=%d,%d,%d\r\n",(unsigned short)tR,(unsigned short)tG,(unsigned short)tB);

//	if((PanelID == CMO_M220Z1_L03)&&(bFactoryMode == FALSE)){
/*	if(PanelID == CMO_M220Z1_L03){
		R_OFFSET_MID = 0x80;
		G_OFFSET_MID = 0x80;
		B_OFFSET_MID = 0x79;
	}
...
//	}
	...
	WriteIIC560(SRGB_COEF_OFFSET, ((unsigned short)ucBkOffset[0]*RGB_OFFSET_SCOPE)/255+RGB_OFFSET_MIN+(ucBlackLevel-ucBrightnessLevel-R_OFFSET_MID)+PanelRGB_Offset);
	
	WriteIIC560(0x1d0,((1 * 2) + 0x21));	
	WriteWordIIC560(0x1d1,0);
	WriteWordIIC560(0x1d3,tG);
	WriteWordIIC560(0x1d5,0);
	WriteIIC560(SRGB_COEF_OFFSET, ((unsigned short)ucBkOffset[1]*RGB_OFFSET_SCOPE)/255+RGB_OFFSET_MIN+(ucBlackLevel-ucBrightnessLevel-G_OFFSET_MID)+PanelRGB_Offset);

	WriteIIC560(0x1d0,((2 * 2) + 0x21));
	WriteWordIIC560(0x1d1,0);
	WriteWordIIC560(0x1d3,0);
	WriteWordIIC560(0x1d5,tB);
	WriteIIC560(SRGB_COEF_OFFSET, ((unsigned short)ucBkOffset[2]*RGB_OFFSET_SCOPE)/255+RGB_OFFSET_MIN+(ucBlackLevel-ucBrightnessLevel-B_OFFSET_MID)+PanelRGB_Offset);

//	if((ucColorTemperature == COLOR_USER)||(ucColorTemperature == COLOR_ORIGIN))
//	if((PanelID == LPL_LM170ME03_TLLX)&&(ucResolution == R640x480))
/*
	if(PanelID == LPL_LM170ME03_TLLX)
		WriteIIC_WaitV(SCALER_I2C_ADDR,0xd0,0x2D);
	else if(PanelID == CMO_M220Z1_L03)
		WriteIIC_WaitV(SCALER_I2C_ADDR,0xd0,0x3D);
	else
		WriteIIC_WaitV(SCALER_I2C_ADDR,0xd0,0x0D);
*/
#if PBusProtocol
	WriteIIC_WaitV(SCALER_I2C_ADDR,0x1D0,0x2d);
#else
	WriteIIC_WaitV(SCALER_I2C_ADDR,0xD0,0x2d);
#endif
/*
	...
	{
		WriteIIC_WaitV(SCALER_I2C_ADDR,0xd0,0x2D);	//NSTL Quality : 640x480 Dot Pattern Flicker
	#if PanelID==LPL_LM170ME03_TLLX
		WriteIIC560(SRGB_DITH_CTRL,0xC0);
	#else	//#if PanelID==LPL_LM190E08_TLLX
		WriteIIC560(SRGB_DITH_CTRL,0xA0);
	#endif
	}
	else
	{
		WriteIIC_WaitV(SCALER_I2C_ADDR,0xd0,0x0D);	//NSTL Quality
	#if PanelID==LPL_LM190E08_TLLX// || PanelID == HSD_HSD190MEN3_A	//xyy 20070524 +HSD
		WriteIIC560(SRGB_DITH_CTRL, 0x00);
	#elif PanelID == HSD_HSD190MEN3_A  //xyy 20070525 +HSD
		WriteIIC560(SRGB_DITH_CTRL, 0x10);
	#else
		WriteIIC560(SRGB_DITH_CTRL, 0xE0);
	#endif
	}
*/
}
