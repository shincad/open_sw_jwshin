...
unsigned char i;
	if(ucOSDType == MAIN_MENU){
		...
		case mSETUP:
			...
			case mSetup_AudioMute:
				...
				if(bMute){
					...
					//---------------- +jwshin 120228 음 차단을 위해 강제로 Write한 PWM Value를 다시 복귀...
		...
		//------------- -jwshin 120224    Direct Menu는 Volume만 처리한다...
	...
	//------------------- +jwshin 120228
		...
		if(bInSync == TRUE) {				// Sync Signal이 있을 경우에는 OSD_OFF() 실행...
			OSD_OFF();
		}
		return;
	}
...
unsigned char i;
	...
	//--------------- RGB Signal Switching 을 위한 Routine... +jwshin 120224
	else if(ucOSDType == POWER_ON_LOGO){
		if(Read24C16(EPADDR_RGB_SW1)) {			// RGB 1 일 때, RGB 2로 Change...
			RGB_SW1_RGB2();
			Write24C16(EPADDR_RGB_SW1,0);
		}
		else {															// RGB 2 일 때, RGB 1으로 Change...
			RGB_SW1_RGB1();
		...
		ucOSDType = POWER_ON_LOGO;		  // +jwshin 120227 ShowSourceMsg 이후, ucOSDType 을 POWER_ON_LOGO 모드로 전환하여 Nosignal 상태에서도 안정화 될 수 있도록 조치...
...
//unsigned char i;
	...
	case FACTORY:
		FactoryEnter();
		break;
	case POWER_ON_LOGO:								// +jwshin 120227 신호 없을 경우에만 .. 모드 전환 부분에서 Menu key disable...
