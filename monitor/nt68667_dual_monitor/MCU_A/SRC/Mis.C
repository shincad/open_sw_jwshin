...
void CheckTimer()
{
	...
	if((ucOSDType <= LF_MAIN_MENU)&&(ucOSDType > OSDOFF) || (ucOSDType==SOURCE_MSG)||							// +jwshin 120224 
	(ucOSDType==LOCK_MSG)||	(ucOSDType==BEST_MSG) || (ucOSDType==DDCCI_MSG) ){			
	...
	//-------------- +jwshin 120302
		...
		if(Get_usWarningMsgTimer_1s() == 0){		// disappear for 60 seconds
			...
			//if ( Get_usWarningMsgTimer_1s()%60==0)
			//printf("usWarningMsgTimer_1s = %d\r\n",(unsigned short)usWarningMsgTimer_1s);
