...
void setup() {
  ...
  attachInterrupt(INT0,togglefunc,FALLING);
...
/*    matrix.setRotation(3);
    ...
    for (int8_t x=7; x>=-100; x--) {
      ...
      matrix.print("YongSan Technical");
      matrix.writeDisplay();
      delay(4000);
      if(buttonState != 4) {
        break;
...
*/
    ...
    for(int8_t x=0;x>=-57;x--) {
      matrix.clear();
      matrix.setCursor(x,0);
      matrix.print("Fighting! ");
      matrix.writeDisplay();
      delay(4000);
      if(buttonState != 4) {
        break;
